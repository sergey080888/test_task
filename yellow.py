...
class ParserYellow:
    ...
    def finding_withoutage_country(self):
        ...
        for country in self.countries_dict:
            params = {'nationality': country, 'resultPerPage': 160}
            ...
            if response.json()['total'] != response_2.json()['total']:
                withoutage_person_list_countries.append(response_2.json()["query"]['nationality'])
    ...
    def finding_withoutage_person(self, url, withoutage_country, headers, params_2):
        ...
        for sexid in sexidlist:
            for symb in self.param_list:
                params = {'name': symb['name'], 'sexId': sexid, 'nationality': withoutage_country,
                          "resultPerPage": 200}
            ...
            for symb in magic_symbols:
                print(symb)
                params = {'name': symb, 'sexId': sexid, 'nationality': withoutage_country, "resultPerPage": 200}
                response = requests.get(url, params=params, headers=headers)
            ...
            print(withoutage_person_list_countries_3)
            for yellow_notice in withoutage_person_list_countries_3:
...
# словарь со странами
    ...
    #
    # param_list = [
    #     {'page': 1, 'resultPerPage': 160, 'ageMin': 32, 'ageMax': 32, 'sexId': 'M', 'nationality': 'RU'},
    #     {'page': 1, 'resultPerPage': 160, 'ageMin': 33, 'ageMax': 33, 'sexId': 'M', 'nationality': 'RU'}]
