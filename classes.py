...
class ParserRed:
   ...
   # словарь со странами
    ...
    #         self.json_saving(wanted_list, country_id)
    #         print(wanted_list)
class ParserYellow(ParserRed):
    def get_total(self):
        super().get_total().url = "https://ws-public.interpol.int/notices/v1/yellow"

    def making_country_dict(self):
        super().making_country_dict().id_ = 'nationality'

    def make_dir(self):
        super().make_dir().name_dir = 'yellow_notice'

    def json_saving(self, list_, id_):
        super().json_saving().directory = 'yellow_notice'
    ...
    def make_json(self):
        ...
        # for country in self.big_wanted_list:
        # self.big_json(country)
        AR = threading.Thread(target=self.big_json, args=('AR',))
        AR.start()

        SV = threading.Thread(target=self.big_json, args=('SV',))
        SV.start()

        IN = threading.Thread(target=self.big_json, args=('IN',))
        IN.start()

        PK = threading.Thread(target=self.big_json, args=('PK',))
        PK.start()

        RU = threading.Thread(target=self.big_json, args=('RU',))
        RU.start()

        US = threading.Thread(target=self.big_json, args=('US',))
        US.start()
